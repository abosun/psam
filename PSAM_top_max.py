...
pb_base_name = 'models/SA101_top_max_s1_'
...
TOP_DIR = '/home/ss/ucf_101_img_test10_top8_npy'
...
def attention0(X, name):
    ...
    slim.losses.add_loss(tf.nn.l2_loss(B)*0.0005)
...
def attention_moduel(X, name):
    ...
    with slim.arg_scope([slim.conv2d], activation_fn=None,
              ...
              weights_regularizer=slim.l2_regularizer(0.005)):
        B = slim.conv2d(X, c, [1, 1],activation_fn=tf.nn.relu, scope='conv2c'+name)
...
with tf.Session(config=cuda_set(GPU)) as sess:
    ...
    Z = slim.max_pool2d(X, [8, 8], scope='pool1')
        ...
        if i_%print_item ==1 :
            ...
            for i in range(data_base_test.test_batch_n+1):
                acc_test,test_loss, test_cross = sess.run([accuracy ,total_loss, loss_cross],
                    feed_dict={X:test_batch_list['top'][i],
