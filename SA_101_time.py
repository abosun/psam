...
MODEL_DIR = 'models/'
SA_MODEL_FILE = 'SAH_ns_mid_v1_0.858.pb'
...
def main():
    ...
    avilist = read_split()
    # print(avilist)
    sess1 = tf.Session(config=cuda_set(gpu=GPU), graph=graph2)
    sess2 = tf.Session(config=cuda_set(gpu=GPU), graph=graph1)
#         names = [op.name for op in sess.graph.get_operations()]
        # print(names)
#         result = []
    v3_mid = sess1.graph.get_tensor_by_name("mixed_2/join:0")
    v3_neck = sess1.graph.get_tensor_by_name("pool_3/_reshape:0")
    v3_top = sess1.graph.get_tensor_by_name("mixed_8/join:0")
    v3_img = sess1.graph.get_tensor_by_name("DecodeJpeg:0")
    v3_out = sess1.graph.get_tensor_by_name("softmax:0")

    sa_neck = sess2.graph.get_tensor_by_name("neck:0")
    sa_top = sess2.graph.get_tensor_by_name("top:0")
    sa_mid = sess2.graph.get_tensor_by_name("mid:0")
    if_train = sess2.graph.get_tensor_by_name("if_train:0")
    soft_tensor = sess2.graph.get_tensor_by_name("softmax:0")
    def get_v3_out(input_img):
        time0 = time.clock()
        neck, top, mid =  sess1.run([v3_neck ,v3_top, v3_mid],feed_dict = {v3_img:input_img})
        time1 = time.clock()
        out = sess2.run([soft_tensor],feed_dict = {sa_neck:neck, sa_top:top, sa_mid:mid, if_train:False})
        time2 = time.clock()
        #print(time2-time0, time1-time0, time2-time1)
        return time2-time0, time1-time0, time2-time1
#         return out[0][0], neck, top, mid
#     def get_v3_out(input_img):
#         time0 = time.clock()
#         _=  sess.run([v3_out],feed_dict = {v3_img:input_img})
#         time1 = time.clock()
#         print(time1-time0)
    for avi in avilist:
        print(avi)
        ...
        for i in range(sum_frame):
            ...
            for i in range(10):
                times.append(get_v3_out(img_list[i]))
#             raise ValueError
...
if __name__ == '__main__' :
    ...
    times = main()
    print(np.median(np.array(times),axis=0))