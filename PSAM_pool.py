...
pb_base_name = 'models/SA101_midpool_s1_'
...
with tf.Session(config=cuda_set(GPU)) as sess:
    ...
    Z = tf.reduce_mean(Y, [1,2])
    #Z = tf.squeeze(Z)
    print(Z)
    #Z = PSAM_Fusion(X, Y)
    # Z = PSAM_TOP(Y)
    Z = slim.dropout(Z, 1, is_training=is_train,scope='fc_drop')
        ...
        if i_%print_item ==1 :
            ...
            for i in range(data_base_test.test_batch_n+1):
                acc_test,test_loss, test_cross = sess.run([accuracy ,total_loss, loss_cross],
                    feed_dict={ Y:test_batch_list['mid'][i],
