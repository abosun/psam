...
# 视频数据
IMG_DTR = 'ucf_101_img_centre'
OUT_DIR_mid = IMG_DTR+'_mid35'
OUT_DIR_top = IMG_DTR+'_top8'
OUT_DIR_neck = IMG_DTR+'_NECK_t'
...
def main():
    ...
    print(len(dataset_dict.keys()))
    for action in dataset_dict.keys():
        gpu_i = (gpu_i+1)%4
        pool.apply_async(read_i, (dataset_dict, action,str(gpu_i)))
    pool.close()
    pool.join()
def read_i(dataset_dict, action, gpu_id):
    ...
    path_graph_def = "models/inception_v3.ckpt"  
    ...
    def cuda_set(gpu='1'):
        ...
        config.gpu_options.allow_growth = True      #程序按需申请内存  
        return config
    network_fn = nets_factory.get_network_fn(model_name, weight_decay=0.0,num_classes=1001,is_training=False)
    img_tensor = tf.placeholder(dtype=tf.float32, shape=[None, 299, 299, 3], name='image_tensor')
    logits, end_points = network_fn(img_tensor)
    sess = tf.Session(config=cuda_set(gpu=gpu_id))
    saver = tf.train.Saver()
    saver.restore(sess,path_graph_def)
    neck_tensor = sess.graph.get_tensor_by_name('InceptionV3/Logits/AvgPool_1a_8x8/AvgPool:0')
    ...
    for file_path in file_path_list:
        ...
        img = (img-128)/128
        neck_values = sess.run([neck_tensor], {img_tensor: [img]})
        # write neck
        out_path_neck = os.path.join(out_dir_neck, os.path.basename(file_path))+'.txt'
