...
def avi2list(avi, label_dict):
    ...
    glob_path = os.path.join(IMG_DTR, '*'.join(avi.split('['))) + '*.jpg'
...
def split_list(avilist):
    lenth = len(avilist)
    sub_len = int(ceil(lenth/jc))
    result = [0]*jc
    for i in range(jc-1):
        result[i] = avilist[i*sub_len:(i+1)*sub_len]
    result[jc-1] = avilist[(jc-1)*sub_len:]
    return result 

def read_i(GPU,avilist):
    ...
    with tf.Session(config=cuda_set(gpu=GPU), graph=graph) as sess:
        ...
        for avi in avilist:
            print(avi)
            img_list, video_n, class_n = avi2list(avi, label_dict)
            target_path = os.path.join(dir_list[-1], class_n, video_n)
            q_list = glob.glob('*'.join(target_path.split('['))+'*txt')
            if len(q_list)>0:
                continue
            class_id = label_dict[class_n]
            if len(img_list) != sum_frame :
                print(avi)
                print(len(img_list))
                print(sum_frame)
                raise ValueError
...
def main():
    # print(label_dict)
    avilist = read_split()
    split_sub = split_list(avilist)
    # print(avilist)
    pool = multiprocessing.Pool(processes=jc)
    for i in range(jc):
        pool.apply_async(read_i, (str(i%4), split_sub[i]))
    pool.close()
    pool.join()
