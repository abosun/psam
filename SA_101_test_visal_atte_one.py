...
def read_split():
    ...
    return split_set
def pay_atte0(frame, w, a):
    frame[:,:,0] = frame[:,:,0] * a
    frame[:,:,1] = frame[:,:,1] * a
    frame[:,:,2] = frame[:,:,2] * np.maximum(w*0.9, a)
    return frame
def pay_atte(frame0, w, a):
    frame = frame0.copy()
    frame[:,:,0] = frame[:,:,0] * a
    frame[:,:,1] = frame[:,:,1] * a
    frame[:,:,2] = frame[:,:,2] * np.maximum(w*0.95, a)
    return frame
def pay_atte2(frame0, w1, w2, a):
    frame = frame0.copy()
    frame[:,:,0] = frame[:,:,0] * a
    frame[:,:,1] = frame[:,:,1] * a
    #w2[218:239,122:143] = w2[218:239,122:143]*2
    #w2[:188,:] = w2[:188,:]*0.1
    #w2[187:215,155:223] = w2[187:215,155:223]*1.3
    #w2[220:242,179:206] = w2[220:242,179:206]*1.3
    #w2 = np.power(w2, 0.7)
    frame[:,:,2] = frame[:,:,2] * np.maximum(w1*0.8+w2*1, a)
    frame[:,:,2] = np.minimum(frame[:,:,2],254.0)
    return frame
def resize299(img8):
    big = np.max(img8)
    img = img8 / big
...
def get_atte0(sess, file):
    ...
    img = pinghua(img)
    frame_w = pay_atte0(frame, img, 0.2)
    return frame_w
def get_atte1(sess, file):
    ...
    frame = get_one_img(file)[0]
    img = resize299(attention_map[0][0])
    #img[205:,153:] = img[205:,153:]*2.2
    #img = img*1.3
    #img[243:260,150:] = img[243:260,150:]*0.3
    #img[260:271,167:] = img[260:271,167:]*0.3
    #img[271:285,180:] = img[271:285,180:]*0.3
    #img[283:,200:] = img[283:,200:]*0.3
    #img[:210,182:] = img[:210,182:]*0.3
    #img[:230,:123] = img[:230,:123]*0.3
    #img = img * 0.5
    #img[168:,:230] = img[168:,:230]*3
    #img[:210,:] = img[:210,:]*0.4
    img = pinghua(img)
    #img[168:,:230] = img[168:,:230]*2
    #img = pow(img,1.34)
    frame_w = pay_atte(frame, img, 0.2)
    img2_path = glob.glob(file+'*ora*.npy')[0]
    print(img2_path)
    img2 = np.load(img2_path)
    frame_w2 = pay_atte2(frame, img, img2, 0.2)
    return frame_w, frame_w2
def draw_i(name, GPU):
    graph_0 = tf.Graph()
...
def draw_one(name,file1, GPU):
    ...
    file_set = set([os.path.basename(path).split('.')[0] for path in file_list])
    for (i,file) in enumerate(file_set):
        if not file==file1: continue
        frame_w_0 = get_atte0(sess_0, file)
        frame_w_1,frame_w_2 = get_atte1(sess_1, file)
        frame = get_one_img(file)[0]
        print(np.max(frame))
        print(np.max(frame_w_0))
        print(np.max(frame_w_1))
        #print(np.max(frame_w_2))
        cv.imwrite('xkeshi31'+file+'.jpg',np.concatenate((frame, frame_w_0, frame_w_1, frame_w_2), axis=1))


if __name__ == '__main__' :
    # get_center_frame(data_path)
    # main()
    draw_one('Surfing', 'v_Surfing_g06_c02','0')
    #v_Diving_g09_c02
    #v_Diving_g12_c06
    #v_BlowingCandles_g05_c01 0.55 0.55 1.13
    #v_Biking_g15_c02 0.9 0.9 0.5 1.65
    #v_HorseRiding_g16_c06 0.9 0.6 0.23 1.23
