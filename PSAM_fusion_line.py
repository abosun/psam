...
pb_base_name = 'models/SA101_linef_s1_'
...
TOP_DIR = '/home/ss/ucf_101_img_test10_top8_npy'#'ucf_101_img_mul12_top8'
MID_DIR = '/home/ss/ucf_101_img_test10_mid35_npy'
...
class Data_base():
    ...
    def next_batch(self, batch_size):
        ...
        for kk in self.features:
            self.data_batch[kk] = [read_data(self.path_list[kk][i], self.tensor_size[kk]) for i in rand_list]
...
def attention_moduel(X, name):
    ...
    with slim.arg_scope([slim.conv2d], activation_fn=None,
        ...
        B = slim.conv2d(B, 1, [1, 1],activation_fn=tf.nn.softmax, scope='conv21'+name)
        B = tf.reshape(B, [-1, n*n, 1], name='Attention_'+name)
    slim.losses.add_loss(tf.nn.l2_loss(B)*0.0001)
    Z = tf.matmul(tf.transpose(tf.reshape(X,[-1,n*n,f]), perm=[0,2,1]),B)
    Z = tf.reshape(Z, [-1,f], name='features'+name)
...
data_base = Data_base(NECK_DIR, TOP_DIR, MID_DIR, OUT_DIR, 'npy')
...
with tf.Session(config=cuda_set(GPU)) as sess:
    ...
    Z_x, Z_y = PSAM_2(X, Y)
    # Z = PSAM_TOP(Y)
    Z_x = slim.dropout(Z_x, 0.8, is_training=is_train,scope='fc_drop')
    Z_x = tf.expand_dims(tf.expand_dims(Z_x,1),1)
    Z_y = slim.dropout(Z_y, 0.8, is_training=is_train,scope='fc_drop')
    Z_y = tf.expand_dims(tf.expand_dims(Z_y,1),1)
    with slim.arg_scope([slim.conv2d], activation_fn=None,
              weights_initializer=tf.truncated_normal_initializer(0.0, 0.01),
              weights_regularizer=slim.l2_regularizer(0.01)):
        logits_X = slim.conv2d(Z_x, c, [1, 1], scope='fc_X')
        logits_Y = slim.conv2d(Z_y, c, [1, 1], scope='fc_y')
    logits_Z = (logits_X + ky * logits_Y) / (1 + ky)
    logits_Z = tf.squeeze(logits_Z)
