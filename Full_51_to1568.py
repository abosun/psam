...
def graph_def(net,is_train):
    with slim.arg_scope([slim.fully_connected], #activation_fn=tf.nn.tanh,
        ...
        net = tf.nn.tanh(net, name='feature')
    with slim.arg_scope([slim.fully_connected], #activation_fn=tf.nn.tanh,
                      weights_initializer=tf.truncated_normal_initializer(0.0, 0.02),
...
def main():
    ...
    with tf.Session(config=cuda_set(gpu=GPU)) as sess:
        ...
        for i in range(ITEMS):
            # print(i)
            train_batch, label_batch = get_train_batch_all(train_path_list, train_label_list)
            # train_batch, label_batch = get_train_batch(train_path_list, train_label_list, test_set)
            if (i+1)%500 == 0 :
                _, loss, acc = sess.run([train_op, total_loss, accuracy],
                        feed_dict={input_tensor:train_batch, label_index:label_batch, is_train:True})
                # test_acc, test_loss = sess.run([accuracy, total_loss], feed_dict={input_tensor:test_batch, label_index:test_label_batch, is_train:False})
                print("step %6d  loss=%f  acc=%f "%(i+1, loss, acc))
                to_pb.save_max(i)
            else:
                sess.run([train_op], feed_dict={input_tensor:train_batch, label_index:label_batch, is_train:True})
