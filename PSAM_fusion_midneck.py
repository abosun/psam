...
pb_base_name = 'models/SA101_neckmid1_s1_'
...
class Data_base():
    def __init__(self, neck_dir, top_dir, mid_dir, out_dir, ext):
        print("Reading data ...")
        self.ext = ext
        self.extdict = {'mid':'npy', 'neck':'txt'}
        self.features = ['mid','neck']
    ...
    def next_batch(self, batch_size):
        ...
        for kk in self.features:
            self.data_batch[kk] = [read_data(self.path_list[kk][i], self.tensor_size[kk], self.extdict[kk]) for i in rand_list]
    ...
    def read_dir(self, class_name, kk):
        class_index = self.label_dict[class_name]
        top_glob = os.path.join(self.file_dir[kk], class_name, "*"+self.extdict[kk])
...
def attention_moduel(X, name):
    ...
    with slim.arg_scope([slim.conv2d], activation_fn=None,
              weights_initializer=tf.truncated_normal_initializer(0.0, 0.01),
              weights_regularizer=slim.l2_regularizer(0.005)):
        B = slim.conv2d(X, c, [1, 1],activation_fn=None, scope='conv2c'+name)
...
#    slim.losses.add_loss(pre_cross*0.1)
    ...
    for i_ in range(ITEMS):
        ...
        _, loss,test_cross, acc2 = sess.run([train_op, total_loss, loss_cross, accuracy],
            feed_dict = {Y:next_batch['mid'], N:next_batch['neck'],
