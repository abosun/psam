...
# 视频数据
IMG_DTR = 'ucf_101_img_centre_jun'
...
def main():
    ...
    top_tensor, mid_tensor, img_tensor, jpeg_data_tensor = tf.import_graph_def(graph_def, return_elements=[TOP_TENSOR_NAME, MID_TENSOR_NAME, IMG_TENSOR_NAME,JPEG_DATA_TENSOR_NAME])
    category='training'
    k=8
    ...
    for action in dataset_dict:
        ...
        for file_path in dataset_dict[action]:
            ...
            if os.path.exists(out_path_top):continue
            img = cv.imread(file_path)
            top_values, mid_values = sess.run([top_tensor, mid_tensor], {img_tensor: img})
            ...
            if not ( os.path.exists(out_path_top) and os.path.exists(out_path_mid) ) :
                raise ValueError
