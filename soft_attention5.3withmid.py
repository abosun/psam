...
class Data_base():
    ...
    def get_test(self, test_batch_size):
        ...
        self.test_lenth_list = [test_batch_size]*list_n + [self.test_lenth % test_batch_size]
        result = [0]*self.test_lenth

        pool = multiprocessing.Pool(processes=jc)
        for test_i in range(self.test_lenth):
                result[test_i] = pool.apply_async(self.read_i_test, (test_i, ))
        pool.close()
        pool.join()
        result_n = [0]*(list_n+1)
        for test_n_i in range(list_n+1):
            result_one = [0]*self.test_lenth_list[test_n_i]
            result_one_k = [0]*self.test_lenth_list[test_n_i]
            for i in range(test_n_i*test_batch_size, test_n_i*test_batch_size+self.test_lenth_list[test_n_i]):
                result_one[i-test_n_i*test_batch_size] = result[i].get()
            for kk in self.one_sample.keys():
                for i in range(test_n_i*test_batch_size, test_n_i*test_batch_size+self.test_lenth_list[test_n_i]):
                    # print(result_one[i-test_n_i*test_batch_size])
                    result_one_k[i-test_n_i*test_batch_size] = result_one[i-test_n_i*test_batch_size][kk]
                self.data_test_batch[kk].append(result_one_k)
...
with tf.Session(config=cuda_set(GPU)) as sess:
    ...
    for i_ in range(ITEMS):
        ...
        next_batch = data_base.next_batch_mul(batch_size)
