...
ITEMS = 3000
drop_rate = 0.9
SPLIT_PATH = 'hmdbTraintestsplits/hmdball_path_list.txt'
LABEL_PATH = 'hmdbTrainTestlist/classInd.txt'
MODEL_DIR = 'models/'
MODEL_FILE = 'SAH_ns_mid_v1_0.858.pb'
MODEL_FILE_v3 = 'classify_image_graph_def.pb'
MODEL_FILE_at = 'SA51_rtr_JT_s1_10751.pb'
GPU = '0'
IMG_DIR = 'HMDB51_img_mul10'
...
file = 'v_WalkingWithDog_g20_c02'
DIR_towrite = 'HMDB51_max_feat_test_10'
...
def main():
    ...
    with graph_v3.as_default():
        with gfile.FastGFile(os.path.join(MODEL_DIR, MODEL_FILE_v3), 'rb') as f:
            graph_def = tf.GraphDef()
            graph_def.ParseFromString(f.read())
            tf.import_graph_def(graph_def, name='')
    sess_v3 = tf.Session(config=cuda_set(gpu=GPU), graph=graph_v3)

    neck_v3 = sess_v3.graph.get_tensor_by_name("pool_3/_reshape:0")
    top_v3 = sess_v3.graph.get_tensor_by_name("mixed_8/join:0")
    img_v3 = sess_v3.graph.get_tensor_by_name("DecodeJpeg:0")

    graph_at = tf.Graph()
    with graph_at.as_default():
        with gfile.FastGFile(os.path.join(MODEL_DIR, MODEL_FILE_at), 'rb') as f:
            graph_def = tf.GraphDef()
            graph_def.ParseFromString(f.read())
            tf.import_graph_def(graph_def, name='')
    sess_at = tf.Session(config=cuda_set(gpu=GPU), graph=graph_at)

    names = [op.name for op in sess_at.graph.get_operations()]

    neck_at = sess_at.graph.get_tensor_by_name("neck:0")
    top_at = sess_at.graph.get_tensor_by_name("top:0")
    softmax = sess_at.graph.get_tensor_by_name("softmax:0")
    feat_tensor = sess_at.graph.get_tensor_by_name("ZN_comb:0")

    if_train = sess_at.graph.get_tensor_by_name("if_train:0")

    for id,name in enumerate(read_split()):
        imgs, path = get_one_img('*'.join(name.split('[')))
        dir_name = os.path.basename(os.path.dirname(path))
        dir_targ = os.path.join(DIR_towrite, dir_name)
        if not os.path.exists(dir_targ): os.makedirs(dir_targ)
        neck  = [0]*8
        top  = [0]*8
        for i in range(8):
            a, b = sess_v3.run([neck_v3, top_v3], feed_dict = {img_v3: imgs[i]})
            neck[i] = a[0]
            top[i] = b[0]
        score, feat = sess_at.run([softmax, feat_tensor], feed_dict = {neck_at: neck, top_at:top, if_train:False})
        a = score.transpose()[label_dict[dir_name]]
        index = np.argmax(a)
        tar_path = os.path.join(dir_targ, name+'.cnnfeat.txt')
        with open(tar_path,'w') as f:
            f.write(' '.join([str(x) for x in feat[index].tolist()]))
        print(str(id)+' '+tar_path)
        ...
        # total = 0.0
        # jishu = [0]*12
        # neck_v3 = sess.graph.get_tensor_by_name("neck:0")
        # top_v3 = sess.graph.get_tensor_by_name("neck:0")
        ...
        # # print(attention_map)
        # frame = get_one_img(file)[-1]
        # img = resize299(attention_map[0][-1])
        # frame[:,:,0] = frame[:,:,0] * img
        # frame[:,:,1] = frame[:,:,1] * img
        # frame[:,:,2] = frame[:,:,2] * img
