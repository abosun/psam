...
def main():
    ...
    with tf.Session(config=cuda_set(gpu=GPU), graph=graph) as sess:
        ...
        v3_out = sess.graph.get_tensor_by_name("softmax:0")

        # sa_neck = sess.graph.get_tensor_by_name("neck:0")
        # sa_top = sess.graph.get_tensor_by_name("top:0")
        # sa_mid = sess.graph.get_tensor_by_name("mid:0")
        ...
        #     time1 = time.clock()
        #     out = sess.run([soft_tensor],feed_dict = {sa_neck:neck, sa_top:top, sa_mid:mid, if_train:False})
        #     time2 = time.clock()
