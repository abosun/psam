...
pb_base_name = 'models/SA101_Stop_s1_preloss_'
...
with tf.Session(config=cuda_set(GPU)) as sess:
    ...
    N_d = tf.expand_dims(tf.expand_dims(N,1),1)
    N_d = slim.dropout(N_d, 0.8, is_training=is_train, scope='pres_drop')
    with slim.arg_scope([slim.conv2d], #activation_fn=tf.nn.softmax,
              weights_initializer=tf.truncated_normal_initializer(0.0, 0.01),
              weights_regularizer=slim.l2_regularizer(0.01)):
        logits_N = slim.conv2d(N_d, c, [1, 1], scope='pre_logit')
    S = tf.nn.softmax(logits_N, dim=-1)
    # Z = PSAM_TOP(X)
    Z = PSAM_STOP(S, X)
    # Z = tf.concat([Z, N], axis=1, name='ZN_comb')s
    ...
    for i_ in range(ITEMS):
        ...
        _, loss,test_cross, acc2 = sess.run([train_op, total_loss, loss_cross, accuracy],
            feed_dict = {N:next_batch['neck'], X:next_batch['top'], C:next_batch['label'], is_train:True})
        ...
        if i_%print_item ==1 :
            ...
            for i in range(data_base_test.test_batch_n+1):
                acc_test,test_loss, test_cross = sess.run([accuracy ,total_loss, loss_cross],
                    feed_dict={N:test_batch_list['neck'][i], X:test_batch_list['top'][i], C:test_batch_list['label'][i], is_train:False})            
