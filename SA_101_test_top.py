...
data_test_dir   = 'ucf_101_img_mul12_top8'
...
MODEL_DIR = 'models/'
MODEL_FILE = 'models/SA101_top_s1_0.827.pb'
GPU = '1'
...
test_dir = {'top':'ucf_101_img_mul12_top8'}
...
with tf.Session(config=cuda_set(gpu=GPU), graph=graph) as sess:
    ...
    total = 0.0
    jishu = [0]*12
    neck = sess.graph.get_tensor_by_name("top:0")
    ...
    for xx in test_data:
        ...
        total += 1
          #s
        soft = sess.run([soft_tensor], feed_dict={neck:data_list['top'], if_train:False})
