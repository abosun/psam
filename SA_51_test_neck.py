...
#OUT_DIR = 'HMDB51_img_mul12_NECK_OUT'
# 
# TOP_DIR = 'HMDB51_img_mul12_top8_max3'
# MID_DIR = 'HMDB51_img_mul12_mid35_max3'
# NECK_DIR = 'HMDB51_img_mul12_NECK_max3'
# OUT_DIR = 'HMDB51_img_mul12_NECK_OUT_max3'
# 
...
def main():
    ...
    with tf.Session(config=cuda_set(gpu=GPU), graph=graph) as sess:
        ...
        for xx in test_data:
            ...
            #print(data_list['neck'])
            soft_fusion = sess.run([soft_tensor], feed_dict={neck:[np.array(data_list['neck']).mean(axis=0)], if_train:False})
            #fusion_label = np.argmax(soft[0].mean(axis=0))
            fusion_label = np.argmax(soft_fusion[0])
            for i in range(12):
