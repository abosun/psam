...
SPLIT_PATH_TEST = 'hmdbTrainTestlist/testlist03.txt'
SPLIT_PATH_TRAIN = 'hmdbTrainTestlist/trainlist01.txt'
LABEL_PATH = 'hmdbTrainTestlist/classInd.txt'
pb_base_name = 'models/SA51_rtr_JT_s3_'
# TOP_DIR_t = 'HMDB51_img_centre_top8'
# MID_DIR_t = 'HMDB51_img_centre_mid35'
# NECK_DIR_t = 'HMDB51_img_centre_NECK'
# OUT_DIR_t = 'HMDB51_img_centre_NECK_OUT'
TOP_DIR_t = 'HMDB51_img_mul12_top8_max'
MID_DIR_t = 'HMDB51_img_mul12_mid35_max'
NECK_DIR_t = 'HMDB51_img_mul12_NECK_max'
OUT_DIR_t = 'HMDB51_img_mul12_NECK_OUT_max'
...
batch_size = 50
c = 51
tensor_shape = {'neck':[2048], 'top':[8,8,1280], 'mid':[35,35,288], 'pre_out':[c]}
ITEMS = 11000
print_item = 50
tight_a = 0.1
large_batch = 1000
...
def load_class(label_path):
    ...
    with open(label_path) as f:
        ...
        while(1):
            if line=='' :break
            data = line.split('\r\n')[0].split(' ')
            label_dict[data[1]] = int(data[0])
            label_dict[int(data[0])] = data[1]
            line = f.readline()
    return label_dict
...
class Data_base():
    def __init__(self, neck_dir, top_dir, mid_dir, out_dir):
        print("Reading data ...")
        self.features = ['neck', 'top']
        self.one_sample = {'neck':[], 'top':[], 'mid':[],'label':[]}
        self.file_dir = {'neck':neck_dir, 'top':top_dir, 'out':out_dir, 'mid':mid_dir}
        self.tensor_size = {'neck':(2048),'top':(8,8,1280),'mid':(35,35,288),'out':(c)}
        self.path_list = {'neck':[], 'top':[], 'mid':[],'label':[]}
        self.data_train = {'neck':[], 'top':[], 'mid':[],'label':[]}
        self.data_test = {'neck':[], 'top':[], 'mid':[],'label':[]}
        self.data_batch = {'neck':[], 'top':[], 'mid':[],'label':[]}
        self.data_test_batch = {'neck':[], 'top':[], 'mid':[],'label':[]}
        self.data_lenth = {'neck_train':[], 'top_train':[], 'out_train':[], 'mid_train':[], 
            'neck_test':[], 'top_test':[], 'out_test':[], 'mid_test':[]}
        self.read_split(SPLIT_PATH_TEST, SPLIT_PATH_TRAIN)
        ...
        for class_name in sub_names:
            ...
            if self.data_lenth['top_train'][-1] != self.data_lenth['neck_train'][-1] or \
                self.data_lenth['top_test'][-1] != self.data_lenth['neck_test'][-1] :
    ...
    def load_class(self, label_path):
        ...
        with open(label_path) as f:
            ...
            while(1):
                if line=='' :break
                data = line.split('\r\n')[0].split(' ')
                self.label_dict[data[1]] = int(data[0])
                self.label_dict[int(data[0])] = data[1]
                line = f.readline()
    ...
    def read_i(self,i):
        ...
        for kk in self.features:
            one_data[kk] = self.read_data(self.path_list[kk][i], self.tensor_size[kk])
            # self.data_batch[kk] = [self.data_train[kk][i] for i in rand_list]
        one_data['label'] = self.data_train['label'][i]
        return one_data
    ...
    def get_test(self, test_batch_size):
        test_data_list = []
        self.test_lenth = len(self.data_test['neck'])
        print('x')
        print(self.test_lenth)
        ...
        self.test_lenth_list = [test_batch_size]*list_n + [test_batch_last]
        print(self.test_lenth_list)
    ...
    def split_list(self, path_list):
        ...
        for path in path_list:
            name = os.path.basename(path).split('.avi')[0]
            if name in self.split_set:
                test_list.append(path)
            else:#if name in self.split_set_train:
                train_list.append(path)
        return train_list, test_list
    def read_split(self,split_path,split_path_train):
        with open(split_path , 'r') as split_file:
            split_string = split_file.read()
        split_list = split_string.split('\r\n')
        self.split_set = set([ os.path.basename(x).split('.avi')[0] for x in split_list])
        print(len(self.split_set))
        with open(split_path_train , 'r') as split_file:
            split_string = split_file.read()
        split_list = split_string.split('\r\n')
        self.split_set_train = set([ os.path.basename(x).split('.avi')[0] for x in split_list])
        print(len(self.split_set_train))
def cuda_set(gpu='3'):
