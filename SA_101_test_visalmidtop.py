...
def main():
    ...
    if_train = sess_att.graph.get_tensor_by_name("if_train:0")
    mid_atte = sess_att.graph.get_tensor_by_name("Attention_mid:0")
    
    img = get_one_img(file)[0]
    Mid, Top, Neck = sess_in3.run([v3_mid, v3_top, v3_neck], feed_dict={v3_img:img})

    Atte_mid = sess_att.run([mid_atte], feed_dict={mid:Mid, top:Top, neck:Neck, if_train:False})
    frame = get_one_img(file)[0]
    print(Atte_mid[0].shape)
